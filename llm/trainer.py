...
class Trainer:
    ...
    def train(self):
        ...
        get_train_batch, get_val_batch = self.load_data(self.config.block_size, self.config.batch_size, self.device)
        stoi = self.tokenizer.stoi
        itos = self.tokenizer.itos

        print(f"Model: {self.config.n_layer}L/{self.config.n_head}H/{self.config.n_embd}D, {sum(p.numel() for p in self.model.parameters()) / 1e6:.1f}M params")
        ...
        for step in range(self.config.max_steps):
            ...
            if step in sample_steps:
                self.model.eval()
                sample = self.model.generate("To be or not", self.tokenizer, max_new_tokens=100, temperature=0.8)
                print(f"\n--- Step {step} sample ---\n{sample}\n---\n")
                if logger:
                    logger.report_text(title="Generated Samples", series="shakespeare", iteration=step, msg=sample)
                self.model.train()
        ...
        if best_step >= 0:
            ...
            ckpt_path = os.path.join(run_dir, "checkpoint_lowest_loss.pt")
            ...
            best_model.eval()
            best_sample = best_model.generate("To be or not", self.tokenizer, max_new_tokens=100, temperature=0.8)
            print(f"\n--- Best sample ---\n{best_sample}\n---\n")
            if logger:
                logger.report_text(title="Best Loss Sample", series="best", iteration=best_step, msg=best_sample)
